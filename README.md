...
# 2. 셋업 스크립트 실행
& "C:\Users\Rich\Desktop\ai-dev-rules\templates\setup-project.ps1" -ProjectPath $PWD
```

스크립트가 자동으로:
- `ai-dev-rules/.ai/core.md` → 프로젝트의 `.ai/core.md`로 심볼릭 링크 생성
- `ai-dev-rules/.ai/rules` → 프로젝트의 `.ai/rules`로 심볼릭 링크 생성
- `templates/entrypoints/`의 각 도구별 진입점 파일(`CLAUDE.md`, `.github/copilot-instructions.md`, `.antigravityrules`)을 프로젝트에 복사
...
각 프로젝트에 복사된 진입점 파일(`CLAUDE.md`, `.github/copilot-instructions.md`, `.antigravityrules`) 하단에 프로젝트 특화 내용을 추가합니다.  
