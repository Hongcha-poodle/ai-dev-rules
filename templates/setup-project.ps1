...
Write-Host "  4. (Claude Code) .claude/settings.json, .claude/agents/, .claude/skills/, CLAUDE.md를"
Write-Host "     새로 만들거나 수정한 뒤에는 세션을 리로드해야 새 harness가 활성화됩니다:"
Write-Host "       /exit  ->  claude --resume  ->  방금 종료한 세션 선택"
Write-Host "     (이 파일들은 세션 시작 시 1회만 로드됩니다)"
Write-Host "  5. 워크플로 업데이트 시 이 스크립트를 다시 실행하면 .ai/ 전체가 업데이트됩니다."
